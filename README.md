...
I bagan by importing the **random** module which will let me initialise a random number to calculate against. A function which I named **sqrt()**, is then defined. The first element is the creation of a random number between 1 and the user's number is generated by the **random.uniform()** function. This is stored a variable **A**. We then use the **range** function to set up 100 iterations of the formula. The function then runs and the result after 100 iterations is returned.
For the main program the user is asked to input a positive number which is converted to the type **float**. **If** it is negative, an error message is returned, otherwise the program carries on. The **user_number** is passed through the function **sqrt()**, and the result is then rounded to 1 decimal place as seemed implied in the brief. The **rounded_result** is then printed out.
...
We then create a **numpy.array** with the **range** (0,5) on the x axis. This will result in values from 0 to 4, as the upper trange is non-inclusive. I felt this was implied in the wording of the brief as opposed to the range being[0,4] which would result in the values from 0 to 3 just.
